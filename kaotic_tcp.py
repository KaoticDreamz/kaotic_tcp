#!/usr/bin/env python3
"""
Kaotic TCP (defaults to port 80, 10000 packets)
...
try:
    ...
    print("Usage: kaotic_tcp.py host [port] [maxCount]")
    sys.exit(1)
...
def getResults():
    ...
    print("\nKAOTIC Ping Results: Connections (Total/Pass/Fail): [{:}/{:}/{:}] (Failed: {:}%)".format((count), passed, failed, str(lRate)))
...
while count < maxCount:
    ...
    if success:
        print("TARGET LOCATED%s[%s]: tcp_seq=%s time=%s ms" % (host, port, (count-1), s_runtime))
        passed += 1
